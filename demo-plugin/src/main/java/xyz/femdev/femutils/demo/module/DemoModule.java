...
import xyz.femdev.femutils.demo.DemoConfig;
...
/**
 * A single module encapsulating the demo plugin's functionality.
 */
public class DemoModule implements Module {
    ...
    public DemoModule(JavaPlugin plugin) {
        this.plugin = Objects.requireNonNull(plugin, "plugin");
    }

    @Override
    public void init(ModuleContext ctx) {
        PaperConfigs cfgs = new PaperConfigs(plugin);
        try {
        ...
        commands = new FemCommandManager(plugin);
        events = new Events(plugin);
        tasks = new Tasks(plugin);
        guiManager = new GuiManager(plugin);

        ctx.getLogger().info("DemoModule initialized");
    }

    @Override
    public void start() {
        registerCommands();
        plugin.getLogger().info("DemoModule started!");
    }
    ...
    public void stop() {
        ...
        quitSubs.clear();
        plugin.getLogger().info("DemoModule stopped and cleaned up");
    }

    private DemoConfig cfg() {
    ...
    private void registerCommands() {
                    ...
                    switch (action) {
                        case "start" -> {
                            ...
                            var sub = events.subscribe(org.bukkit.event.player.PlayerQuitEvent.class, quit ->
                                    plugin.getLogger().info(quit.getPlayer().getName() + " left!"));
        ...
        CommandNode chainDemo = CommandNode.literal("chain")
                ...
                .exec(ctx -> {
                    ...
                    TaskChain chain = tasks.chain()
                            ...
                            .onFinally(() -> plugin.getLogger().info("Chain finished for " + p.getName()));
        ...
        CommandNode anvilCmd = CommandNode.literal("anvil")
                ...
                .exec(ctx -> {
                    Player p = ctx.player();
                    AnvilPrompt prompt = new AnvilPrompt(plugin);
                    prompt.open(
                            p,
